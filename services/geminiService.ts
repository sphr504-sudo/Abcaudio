...
export async function decodeAudioData(
  ...
  // Use DataView or specify offset/length for Int16Array to avoid alignment issues
...
export const analyzeScript = async (text: string): Promise<ScriptAnalysis> => {
  ...
  if (!apiKey) throw new Error("API_KEY is missing. Please add it to your environment variables.");
...
export const synthesizeSegment = async (
  ...
  if (!base64) {
    throw new Error(`Synthesis failed for line: ${segment.text.substring(0, 30)}...`);
  }
  return base64;
};
